        ...
        <form @submit.prevent="calculate">
            <div class="calculator card">
                ...
                <div :class="{ shake: disabled }">
                    <input type="text" id="screen" class="calculator-screen z-depth-1" v-model="expression" disabled />
                </div>
   ...
   methods: {
      inputDigit(digit) {
         ...
         if (!calculator.isResult) {
            ...
            this.$notify({
               type: "error",
               text: "Press AC to start a new operation"
            })
         }
      },
      inputDecimal(dot) {
      ...
      async handleOperator(nextOperator) {
         ...
         if (operator && calculator.waitingForSecondOperand) {
            this.$notify({
               type: "warn",
               text: "You can do only one operation at a time"
            })
            return;
         }
         if (calculator.isResult) {
      ...
      async doOperation() {
         ...
         if (!calculator.isResult && calculator.operator) {
            ...
            if (price <= this.userBalance) {
               ...
               this.$notify({
                  type: "error",
                  text: "Not enough credit to perform this operation"
               })
            }
         } else {
            this.$notify({
      ...
      copy() {
        ...
        if (isMobile) {
            isCopied = this.copyMobile(calculator.displayValue);  
                    if(isCopied){
                        this.$notify({
                            type: "success",
